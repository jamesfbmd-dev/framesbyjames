document.addEventListener('DOMContentLoaded', () => {
    ...
    function appendImages(images) {
        images.forEach((data, index) => {
            ...
            const shortestColumn = columns.reduce((acc, col) => {
                return col.offsetHeight < acc.offsetHeight ? col : acc;
            }, columns[0]);

            const item = document.createElement('div');
            item.className = 'gallery-item';
            // Initially hide the item, then fade it in
            item.style.opacity = '0';
            item.dataset.category = data.category;
            item.dataset.fullSrc = data.src;
            const originalIndex = imageData.findIndex(img => img.src === data.src);
            ...
            contentHolder.addEventListener('click', () => openLightbox(originalIndex));
        });
    }
    ...
    // Responsive handler
    let resizeTimer;
    let lastNumColumns = getNumColumns();

    window.addEventListener('resize', () => {
        clearTimeout(resizeTimer);
        resizeTimer = setTimeout(() => {
            const newNumColumns = getNumColumns();
            if (newNumColumns !== lastNumColumns) {
                lastNumColumns = newNumColumns;
                // Re-render the gallery with the current filter
                applyFilterAndRender(currentFilterType, currentFilterValue);
            }
