...
export function renderGallery(filter = 'all', override) {
...
function createGalleryItem(image) {
    ...
    item.dataset.originalIndex = imageData.findIndex(img => img.src === image.src);
    ...
    item.addEventListener('click', () => {
        import('./lightbox.js').then(module => {
            module.openLightbox(Number(item.dataset.originalIndex));
        });
    });
...
function createImageElement(image, relativeWrapper) {
    ...
    img.alt = image.alt;
    img.loading = 'lazy';
    img.className = image.positionOverride === 'bottom' ? 'img-thumbnail img-bottom' : 'img-thumbnail';
